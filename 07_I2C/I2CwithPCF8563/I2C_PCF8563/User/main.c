...
//    uint8_t dev_addr = 0x3C; // 设备地址7bit I2C屏幕
    ...
    int8_t rst = I2C_write(dev_addr, dev_reg, data, 3);
    ...
    while(1) { 
        ...
        // 读取最新的 秒,分,时 信息
        rst = I2C_read(dev_addr, dev_reg, read_data, 3);
        
        printf("read_data: %02X:%02X:%02X -> %d\n", 
            read_data[2], read_data[1], read_data[0], rst);
