...
//    printf("sendCur_task\n");
    ...
    uint8_t payload[] = {currentAngle >> 8, currentAngle & 0xFF};
...
//    printf("Success: cmd: %02X data_len: %d\n", cmd, data_len);
    ...
    if(cmd == 0xF0){
        targetAngle = (data[0] << 8) | data[1];
    }else if(cmd == 0xE0){
        uint8_t type = data[0];
        float value = bytesToFloat(&data[1]);
        if(type == 0x00){
            kp = value;
        }else if(type == 0x01){
            ki = value;
    ...
    printf("PID kp: %.3f ki: %.3f kd: %.3f target: %d\n",
