...
//	rcu_bkp_reset_disable();
    ...
    rcu_osci_on(RCU_HXTAL); // 高速外部晶振 LXTAL 32.768KHz
    /* 等待晶振稳定 */
...
void RTC_read(){
    ...
    printf("HXTAL %04d-%02d-%02d ", year, month, date);
...
void RTC_write(){
    ...
    rps.factor_syn  = 2499; //15bit 异步分频器[0x0000, 0x7FFF]
    // ck_spre = 1Hz  每秒1Hz
    ...
    // HXTAL 外部高速晶振 8000000/ 25 -------------------------------------
    // ck_spre = 1, rtc_clk = 320,000, factor_a = 127
    // 1 = 32768 / ((0x7F + 1) * (factor_s + 1))
    // factor_s + 1 = 32768 / 128
    // factor_s = 32768 / 128 - 1
    // factor_s = 2499 (0x09C3)
...
int main(void) {
    ...
    if(RTC_BKP1 == 0xAABB){
        printf("时钟时间已经配置过了, 不需要重新写入\n");
    }else {
        printf("首次进入设置, 初始化日期时间: 0x%X\n", RTC_BKP1);
