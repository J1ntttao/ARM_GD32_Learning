...
uint16_t g_recv_buff[ADC_CH_LEN];
...
static void ADC_DMA_config() {
    ...
    /* 设置默认参数 initialize the DMA single data mode parameters struct with the default values */
    ...
    // 启用dma
    dma_flag_clear(ADC_DMA_CH, DMA_FLAG_FTF);
...
static void ADC_config(void)
{
    ...
    */
    ...
    adc_routine_channel_config(ADC0, 0, ADC_CHANNEL_16, ADC_SAMPLETIME_144);
    adc_routine_channel_config(ADC0, 1, ADC_CHANNEL_14, ADC_SAMPLETIME_144);
...
//    adc_software_trigger_enable(ADC0, ADC_ROUTINE_CHANNEL);
    ...
    // 芯片温度 ---------------------------------------------------
    uint16_t adc_val = g_recv_buff[0];
