...
uint16_t adc_channel_sample(uint8_t channel){
    ...
    adc_routine_channel_config(ADC0, 0, channel, ADC_SAMPLETIME_144);
...
void ADC_get_teamperature(){
    ...
    /* 读取常规通道的数据 read ADC routine data register*/
    uint16_t adc_val = adc_channel_sample(ADC_CHANNEL_16);
...
void ADC_get_voltage(){   // 电位器(滑动变阻器)
    /* 读取常规通道的数据 read ADC routine data register*/
    uint16_t adc_val = adc_channel_sample(ADC_CHANNEL_14);
